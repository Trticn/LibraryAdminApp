...
import { changeAuthor, changeQuantity, changeTitle, useAddBookMutation } from "../store";
import Button from "./Button";
import { useToast } from "../context/ToastContext";

function BookAdd({closeModal}) {
  ...
  const handleSubmit = async (e) => {
        ...
        try {
            ...
            showToast({ type: 'success', text: 'Knjiga je uspešno dodata!' });
            closeModal();
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                    <label htmlFor="quantity" className="block text-sm font-medium text-gray-700">Količina:</label>
                    <input
                        type="number"
                        min="1"
                        value={quantity || ''}
